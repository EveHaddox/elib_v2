...
function PANEL:Init()
    ...
    self.save.DoClick = function()
        ...
        // actually save it
        if eve.config.addons[self.addon][self.id].value != tonumber(self.entry:GetValue()) then
            eve.config.addons[self.addon][self.id].value = tonumber(self.entry:GetValue())
    ...
    self.entry:SetWide(80)

    self.entry:SetNumeric(true)

    self.entry.OnLoseFocus = function()
        if self.addon == nil or self.id == nil then return end
        if self.entry:GetInt() != nil and self.entry:GetInt() > eve.config.addons[self.addon][self.id].cfg then
            timer.Simple(0.1, function()
                self.entry:SetValue(eve.config.addons[self.addon][self.id].cfg)
            end)
        end
        self.slider.Fraction = self.entry:GetInt() / eve.config.addons[self.addon][self.id].cfg
        self.slider:InvalidateLayout()
    end

    self.slider = self.addBg:Add("PIXEL.Slider")
    self.slider:Dock(TOP)
    self.slider:DockMargin(12, 8, 12, 0)
    self.slider:SetHeight(15)

    self.slider.OnValueChanged = function(fraction)
        self.entry:SetValue(math.Round(fraction.Fraction * eve.config.addons[self.addon][self.id].cfg))
    end
    ...
    self.textPnl.Paint = function(pnl, w, h)
        ...
        draw.SimpleText("Numeric Slider", "eve.config.popup.small", w / 2, h / 2 + 10, eve.theme.secondaryText, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
function PANEL:SetValue(tbl, id, addon)
    ...
    self.id = id
    self.addon = addon
    self.slider.Fraction = tbl.value / eve.config.addons[self.addon][self.id].cfg
end
...
vgui.Register("eve.config.advanced.int", PANEL, "EditablePanel")