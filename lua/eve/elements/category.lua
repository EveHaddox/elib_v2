...
PIXEL.RegisterFont("UI.CategoryHeader", "Space Grotesk Bold", 19)
...
function PANEL:Paint(w, h)
    ...
    PIXEL.DrawImageRotated(w - h * .3 - PIXEL.Scale(6), h / 2, arrowSize, arrowSize, self.ArrowRotation, "https://pixel-cdn.lythium.dev/i/5r7ovslav", PIXEL.Colors.PrimaryText)
end

vgui.Register("eve.CategoryHeader", PANEL, "PIXEL.Button")
...
AccessorFunc(PANEL, "RemoveBackground", "RemoveBackground", FORCE_BOOL)
AccessorFunc(PANEL, "HeaderHeight", "HeaderHeight", FORCE_NUMBER)

function PANEL:Init()
    self.Header = vgui.Create("eve.CategoryHeader", self)

    self:SetTitle("Eve Category")
...
function PANEL:Paint(w, h)
    if self:GetRemoveBackground() then PIXEL.DrawRoundedBox(PIXEL.Scale(4), 0, 0, w, h, self.BackgroundCol) end
end
...
function PANEL:PerformLayout(w, h)
    ...
    self.Header:SetTall(PIXEL.Scale(self:GetHeaderHeight()))
...
vgui.Register("eve.category", PANEL, "Panel")
